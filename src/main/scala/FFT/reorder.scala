...
class reorder extends Module with Config {
  val io = IO(new Bundle{
    ...
    val dOut = Output(new MyFixComplex)
  ...
  val buffer = RegInit(VecInit(Seq.fill(FFTlength)(0.S((2*DataWidth).W).asTypeOf(new MyFixComplex))))

  def bit_reverse(j: UInt): UInt = {
    var result: UInt = 0.U(FFTbit.W)
    val times = FFTbit
    for (i <- 0 until times) {
      val operator = j & (1.U(FFTbit.W) << i).asUInt
